...
\item student bias update
  \begin{itemize}
  \item report: $\beta_s \leftarrow \beta_s - K (\hat{f}_{st} - f_{st})$
  \item learning: $\beta_s \leftarrow \beta_s + K e^{- K \beta_s}$
  \end{itemize}
...
\item task bias update
  \begin{itemize}
  \item $b_s \leftarrow b_s + \frac{K}{n + K}(\hat{f}_{st} - f_{st})$
